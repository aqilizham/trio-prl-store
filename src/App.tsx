...
import {
  ArrowDown, ArrowRight, Check, ChevronDown, Heart, Menu, Minus, Plus,
  Search, ShoppingBag, Truck, X,
} from 'lucide-react'
...
function App() {
  ...
  const addToCart = (product: Product, size = 'M', amount = 1) => {
    ...
    setSelectedProduct(null); setCartOpen(true); showToast(`${product.name} added to your bag`)
  }
  ...
  return (
    <div className="site-shell">
      ...
      <header className="site-header">
        ...
        <a className="wordmark" href="#top" aria-label="Common Thread home"><span className="wordmark-mark">ct</span><span><strong>COMMON THREAD</strong><small>EVERYDAY UNIFORM</small></span></a>
      ...
      <main id="top">
        <section className="hero-section"><div className="hero-copy"><p className="eyebrow">The new uniform · 2026</p><h1>Good clothes,<br /><em>well considered.</em></h1><p className="hero-description">Clothes for the days that matter and the days that don't. Thoughtful fabrics, easy shapes and a point of view that lasts.</p><a className="dark-button" href="#shop">Explore the collection <ArrowDown size={17} /></a><p className="hero-note"><span className="note-line" /> Free delivery on orders over RM250</p></div><div className="hero-visual"><div className="hero-image-wrap"><img src="/images/common-thread-hero.png" alt="Model wearing a relaxed ivory oxford shirt" /></div><div className="hero-caption"><span>The Everyday Oxford</span><strong>{formatPrice(189)}</strong></div><div className="hero-stamp">Wear it<br /><i>well.</i></div></div></section>
        ...
        <section id="shop" className="shop-section"><div className="section-heading"><div><p className="eyebrow">The collection</p><h2>Pieces with <em>purpose.</em></h2></div><p className="section-intro">A working wardrobe, edited for repeat wear. Every piece designed to settle into your week.</p></div><div className="shop-toolbar"><div className="category-tabs">{categories.map((item) => <button key={item.id} className={category === item.id ? 'active' : ''} onClick={() => setCategory(item.id)}>{item.label}</button>)}</div><span className="result-count">{visibleProducts.length} pieces <ChevronDown size={14} /></span></div><div className="product-grid">{visibleProducts.map((product) => <article className="product-card" key={product.id}><div className="product-image" onClick={() => openProduct(product)}><img src={product.image} style={{ objectPosition: product.imagePosition }} alt={product.name} />{product.label && <span className="product-label">{product.label}</span>}<button className={`favorite-button ${favorites.includes(product.id) ? 'is-favorite' : ''}`} onClick={(event) => { event.stopPropagation(); toggleFavorite(product.id) }} aria-label={`Favourite ${product.name}`}><Heart size={17} fill={favorites.includes(product.id) ? 'currentColor' : 'none'} /></button><button className="quick-add" onClick={(event) => { event.stopPropagation(); addToCart(product) }}>Quick add <Plus size={15} /></button></div><div className="product-info"><div><h3>{product.name}</h3><p>{product.tone}</p></div><strong>{formatPrice(product.price)}</strong></div></article>)}</div></section>
        <section id="about" className="studio-section"><div className="studio-image"><img src="/images/common-thread-knit.png" alt="Navy knitwear and cream tee arranged on a table" /><span>Material study no. 04</span></div><div className="studio-copy"><p className="eyebrow">Our point of view</p><h2>Wear it in,<br /><em>wear it out.</em></h2><p>We believe the best clothes get better with a little history. Common Thread makes reliable staples with subtle details—pieces that meet you where you are, then stay for the next chapter.</p><a className="underlined-link" href="#journal">Read our journal <ArrowRight size={16} /></a><div className="studio-signature">common<br /><i>thread</i></div></div></section>
        <section id="delivery" className="delivery-banner"><div><Truck size={24} /><div><p className="eyebrow">The easy part</p><h2>Take your time.<br /><em>We will take care of the rest.</em></h2></div></div><div className="delivery-details"><p>Free delivery above RM250. Easy returns within 14 days. Need a hand? <a href="mailto:hello@commonthread.my">hello@commonthread.my</a></p><span>Malaysia · Singapore · Worldwide</span></div></section>
      </main>
      <footer id="journal" className="site-footer"><div className="footer-brand"><span className="wordmark-mark">ct</span><p>Good clothes,<br />well considered.</p></div><div className="footer-links"><div><span>Explore</span><a href="#shop">Shop all</a><a href="#shop">New in</a><a href="#about">Our story</a></div><div><span>Help</span><a href="#delivery">Delivery & returns</a><a href="mailto:hello@commonthread.my">Contact</a><a href="#top">Size guide</a></div></div><div className="newsletter"><span>Notes from the thread</span><p>Occasional edits, new pieces and things worth keeping.</p><div className="newsletter-input"><input placeholder="Your email address" aria-label="Email address" /><button aria-label="Subscribe"><ArrowRight size={17} /></button></div></div><div className="footer-bottom"><span>© 2026 Common Thread</span><span>Made slowly in Malaysia</span><span>Instagram · Privacy</span></div></footer>
